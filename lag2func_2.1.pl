...
my @letters = split //, "ABCDEFGHIJKLNOQRSTUVWXYZ";                             # Make an array of letters to use in the line ID (pPmM are illegal)
...
#    $part   =~ s/\(1\+G5\)/PR/g;                                                #   product and parens
    ...
    my $dummyNum  = ( $lineNum < 9999 ) ? $lineNum : "A" . ($lineNum - 10000);
    my @lineID    = map sprintf( '%s%0*s', $_, $digits, $dummyNum ), @letters;  # The line ID numbers in AA000## format
#    my @lineID    = map sprintf( '%s%0*d', $_, $digits, $dummyNum ), @letters;  # The line ID numbers in AA000## format
